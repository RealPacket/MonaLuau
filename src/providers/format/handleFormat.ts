...
export const rangeFormatter: monaco.languages.DocumentRangeFormattingEditProvider = {
	...
	provideDocumentRangeFormattingEdits(
		...
		styluaRange.start = positionToByteOffset(model, range.getStartPosition());
		styluaRange.end = positionToByteOffset(model, range.getEndPosition());
		const formatted = formatCode(text, config, styluaRange, OutputVerification.None);
